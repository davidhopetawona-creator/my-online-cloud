...
async function uploadMyFile() {
    ...
    // 1. Check if the user actually picked a file
    ...
    try {
        // 3. Send it to the API endpoint we created earlier
        ...
        if (response.ok) {
            ...
            statusText.innerText = `Success! Saved as: ${result.fileName}`;
        } else {
            statusText.style.color = "red";
        ...
        statusText.style.color = "red";
        statusText.innerText = "Error connecting to the cloud server.";
        console.error("Upload error:", error);
    }
}
// Automatically download live database settings as soon as a device connects
window.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        if(data && data[0]) {
            // Update the display text for visitors
            document.getElementById('display-title').innerText = data[0].title;
            document.getElementById('display-desc').innerText = data[0].description;

            // Pre-fill the input fields inside the admin dashboard
            document.getElementById('edit-title').value = data[0].title;
            document.getElementById('edit-desc').value = data[0].description;
        }
    } catch (err) {
        console.error("Error syncing content from the live database:", err);
    }
});
